...
1本作成、**masterへ直接push**して公開する（PRは作らない・完全自動公開）。
...
すでにmasterブランチでcheckout済みの状態からスタートする。

1. 記事ファイル `src/content/notes/news-YYYY-MM-DD.mdx` を作成する。
...
4. push後、`.github/workflows/deploy.yml` が自動でビルド・GitHub Pagesへのデプロイを行う。
   追加の作業は不要。
